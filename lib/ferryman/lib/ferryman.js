...
class Ferryman {
    ...
    async createDataHubEntry(applicationUid, recordUid, oihUid, token) {
        try {
            const body = {
                applicationUid,
                recordUid,
            ...
            log.warn(`Failed to post entry to the data-hub via applicationUid ${applicationUid} recordId ${recordUid}`);

            return null;
        } catch (e) {
            log.error(e);
            // eslint-disable-next-line max-len
            log.warn(`Error: Failed to post entry to the data-hub via applicationUid ${applicationUid} recordId ${recordUid}`);
    ...
    async fetchSecret(secretId, token) {
        try {
            const uri = `${this.settings.SECRET_SERVICE_BASE_URL}/secrets/${secretId}`
            logger.trace({secretId, token, uri}, 'fetching secret')
            const options = {
                method: 'GET',
                uri,
                json: true,
    ...
    async addIdMeta(self) {
        let hasOihUid = false;
        // Check if oihUid exists
        if ('id' in self.oihMeta || self.oihMeta.id) {
            const data = await self.getDataHubEntryByOihId(self.oihMeta.id, self.apiKey);

            if (data && data.id) {
                self.oihMeta.id = data.id;
                self.oihMeta.refs = data.refs;
                hasOihUid = true;
            }
            // Check if recordUid exists
        } else if ('recordUid' in self.oihMeta || self.oihMeta.recordUid) {
            const data = await self.getDataHubEntryByRecordId(self.oihMeta.recordUid, self.apiKey);
    ...
    // eslint-disable-next-line consistent-this
      ...
      || !message.properties.headers.orchestratorToken
        ...
        this.oihMeta = (payload.metadata)
            ? payload.metadata : {};

        if (this.settings.DATAHUB_BASE_URL) {
            if (cfg.nodeSettings.idLinking) {
                await this.addIdMeta(self);
            }
        } else {
            log.warn('No DATAHUB_BASE_URL defined skipping ID-Linking functions');
        }


        // Check if we have any recordUid to pass to component
        if (this.oihMeta.refs && Array.isArray(this.oihMeta.refs) && this.nodeSettings && payload.metadata) {
            for (let ref of this.oihMeta.refs) {
                if (ref.applicationUid === this.nodeSettings.applicationUid) {
                    this.oihMeta.recordUid = ref.recordUid;
        ...
        if (tokenData && tokenData.secretId && tokenData.apiKey) {
            ...
            logger.trace('No secret to fetch')
        }

        const stepData = {}; // holdover necessary for compatibility, @todo: remove entirely

        const snapshot = await this.getSnapShot(tokenData.flowId, tokenData.stepId, tokenData.apiKey) || {};
        if (secret) {
            _.assign(cfg, secret);
        }
        ...
        return new Promise((resolve) => {
            ...
            const onData = async (data) => {
                ...
                if (this.nodeSettings && this.nodeSettings.applyTransform
                  && (this.nodeSettings.applyTransform === 'after' || this.nodeSettings.applyTransform === 'both')) {
                    const cfg = {
                        customMapping: this.nodeSettings.secondTransformFunction
                            ? this.nodeSettings.secondTransformFunction : this.nodeSettings.transformFunction
                    };
                    const transformedMsg = transform(message, cfg, false);
                    data = transformedMsg;
                }

                this.oihMeta = (data.metadata)
                    ? data.metadata : {};

                if (this.settings.DATAHUB_BASE_URL && cfg.nodeSettings.idLinking) {
                    let hasOihUid = await this.addIdMeta(self);

                    if (hasOihUid === false) {
                        // Create new oihUid if none found
                        const { recordUid, oihUid } = this.oihMeta;
                        const applicationUid = this.nodeSettings.applicationUid || this.oihMeta.applicationUid;
                        const response = await this.createDataHubEntry(applicationUid, recordUid, oihUid, this.apiKey);
                        if (response && response.id && data.metadata) {
                            this.oihMeta.id = response.id;
                        }
                    }

                    if (this.oihMeta && this.oihMeta.id) {
                        data.metadata.oihUid = this.oihMeta.id;
                    }

                } else if (cfg.nodeSettings.idLinking) {
                ...
                try {
                    ...
                    if (
                        tokenData.nodeSettings
                      && typeof (tokenData.nodeSettings === 'object')
                      && tokenData.nodeSettings.governance
                    ) {
                      await this.generateProvenanceEvent({ // eslint-disable-line
                            oihUid: this.oihMeta.id,
            ...
            async function onRawRecord(data) {
                const headers = _.clone(outgoingMessageHeaders);
                return self.amqpConnection.sendRawRecord(data, headers, self.throttles.snapshot);
            }
